...
from utils.paper_search_tools import SemanticScholarSearch, CoreSearch, OpenReviewSearch, ArXivSearch, normalize_papers
...
class PaperRecommendAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        config = get_config()
        super().__init__(config, agent_name="papers") # Will need to add 'papers' to config if I want specific settings, but base defaults work
        self.recommendation_prompt = get_prompt("paper_recommendation_academic")
        self.qa_prompt = get_prompt("paper_qa_task")
